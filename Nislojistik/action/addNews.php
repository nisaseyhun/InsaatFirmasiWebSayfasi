...
if (isset($_POST['submit'])) {
    ...
    $sql = "INSERT INTO news (news_Title, news_Content, news_Img) VALUES (?, ?, ?)";
    $stmt = $conn->prepare($sql);

    $stmt->bind_param('sss', $title, $description, $imagePath);

    if ($stmt->execute()) {
        echo "<script>alert('Kayıt başarıyla eklendi.'); window.location.href = '../page/admin/listNews.php';</script>";
    } else {
